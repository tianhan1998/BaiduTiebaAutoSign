...
public class Utils {
    ...
    public static boolean oneKeySigned() {
        ...
        if (allUnsigned != null) {
            ...
            if(fail.size()!=0) {
                for (int i = 1; i <= retry_num; i++) {
                    ...
                    List<String> new_failed = new ArrayList<>();
                    signedByList(fail, new_failed, true);
                    if (new_failed.size() != 0) {
                        fail.clear();
                        Collections.copy(fail,new_failed);
                        new_failed.clear();
                    } else {
